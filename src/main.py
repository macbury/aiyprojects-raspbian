...
def do_assistant_library(args, credentials, player, status_ui, local_assistant):
...
installed with:
  ...
  def process_event(event):
    ...
    if event.type == EventType.ON_START_FINISHED:
      ...
      if actor.can_handle(event.args['text']):
        actor.handle(event.args['text'])
      elif local_assistant.process(event.args['text'], say):
        assistant.stop_conversation()
